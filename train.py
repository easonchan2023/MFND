...
class CNN_Fusion(nn.Module):
    ...
    def forward(self, text, image, mask, text1):
        ...
        # image1 = self.vgg(image)  # [N, 512]
        image1 = image_swin_all_final

        #inputs = self.bertModel(text, return_tensors="pt").to("cuda:0")

        #text = self.bertModel.transformer(**inputs, output_hidden_states=True)

        last_hidden_state = torch.mean(self.bertModel(text.long())[0], dim=1, keepdim=False)

        #last_hidden_state = text.last_hidden_state
...
#cross-transformer
        ...
        #text_image_clip = torch.cat((fea_c3, fea_c4, fea_c1, fea_c2), 1)
       # text_image_clip = self.trm(text_image_clip)
        # Fake or real
...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels, event_labels) in enumerate(train_loader):
            ...
            #if min(score_1, score_2, score_3, score_4) == score_1:
               # coeff_1 = 1
                #coeff_2 = 1 - tanh(args.alpha * relu(ratio_2))
               # coeff_3 = 1 - tanh(args.alpha * relu(ratio_3))
                #coeff_4 = 1 - tanh(args.alpha * relu(ratio_4))

            #if min(score_1, score_2, score_3, score_4) == score_2:
                #coeff_1 = 1 - tanh(args.alpha * relu(ratio_1))
               # coeff_2 = 1
               # coeff_3 = 1 - tanh(args.alpha * relu(ratio_3))
               # coeff_4 = 1 - tanh(args.alpha * relu(ratio_4))

           # if min(score_1, score_2, score_3, score_4) == score_3:
               # coeff_1 = 1 - tanh(args.alpha * relu(ratio_1))
               # coeff_2 = 1 - tanh(args.alpha * relu(ratio_2))
                #coeff_3 = 1
                #coeff_4 = 1 - tanh(args.alpha * relu(ratio_4))

           # if min(score_1, score_2, score_3, score_4) == score_4:
               # coeff_1 = 1 - tanh(args.alpha * relu(ratio_1))
               # coeff_2 = 1 - tanh(args.alpha * relu(ratio_2))
               # coeff_3 = 1 - tanh(args.alpha * relu(ratio_3))
               # coeff_4 = 1
